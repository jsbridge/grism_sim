...
def mkobjects_iraf(inimage, outimage, objfile, radius, magzero):
    ...
    if os.path.exists(outimage) == True:     # If the file exists already, IRAF doesn't tell you but just doesn't write the new file
        os.remove(outimage)
        print 'Deleted '+outimage+' first'
    
    iraf.mkobjects(input=inimage, output=outimage, objects=objfile,
            radius=radius, magzero=magzero)
...
def mkobjects_homegrown(infilename, outfilename, objectlistfilename, radius, magzero):
    ...
    x, y, mag, size = np.loadtxt(objectlistfilename, usecols=(0, 1, 2, 4), unpack=True)
